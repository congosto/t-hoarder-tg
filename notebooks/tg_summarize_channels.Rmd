...
msgs_df <- read_csv(
  ...
  mutate(channel_dest = ifelse(
    str_detect(forward_msg_link, "^https://t.me/"), 
      ...
      NA)) %>% 
  mutate(channel_dest_msg = ifelse(
    str_detect(message, "https://t.me/"), 
      str_extract(message, "(?<=https://t.me/)[\\w]+"), 
      NA)) %>% 
  group_by(signature) %>%
  slice(1) %>%
  ungroup() %>%
...
mentions_in <- msgs_df %>%
  filter (!is.na(channel_dest_msg)) %>%
  group_by(channel_dest_msg) %>% 
  summarize(
    count_mentions_in = n(),
    .groups = "drop"
  ) 
mentions_out <- msgs_df %>%
  filter (!is.na(channel_dest_msg)) %>%
  group_by(channel_name) %>% 
  summarize(
    count_mentions_out = n(),
    .groups = "drop"
  ) 
nodos <- metadatos_df %>%
  left_join(forward_in , by = c("username" = "channel_dest")) %>%
  left_join(forward_out , by = c("username" = "channel_name")) %>%
  left_join(mentions_in , by = c("username" = "channel_dest_msg")) %>%
  left_join(mentions_out , by = c("username" = "channel_name")) %>%
  ...
  mutate (count_mentions_in = ifelse(!is.na(count_mentions_in),count_mentions_in, 0)) %>%
  mutate (count_mentions_out = ifelse(!is.na(count_mentions_out),count_mentions_out, 0 )) %>%
  mutate (count_mentions_tot = count_mentions_in + count_mentions_out ) %>%
  filter (count_forward_tot > 0 | count_mentions_tot > 0) %>%
  arrange (desc(count_forward_tot)) %>%
  select (username, count_forward_tot, count_forwards_in, count_forwards_out, count_mentions_tot, count_mentions_in, count_mentions_out,date, created_year, participants_count, log_participants,fake, verified)   # Cambiar orden de las columnas
```
...
head_nodos <- "nodedef> name VARCHAR, tot_forwards INT, in_forwards INT, out_forwards INT, tot_mentions INT, in_mentions INT, out_mentions INT,date_since VARCHAR, year_since VARCHAR, participants_count INT, log_participants INT,  fake BOOLEAN, verified BOOLEAN"
...
write_csv(aristas_forward, name_file_gdf, append = TRUE, col_names = FALSE)
write_csv(aristas_mention, name_file_gdf, append = TRUE, col_names = FALSE)
```
