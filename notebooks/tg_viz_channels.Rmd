...
params: 
    ...
    base_title : "xxxxxxx"               # Prefijo del título principal de las gráficas
    ...
    max_overlaps: 30  
    topics: FALSE                        # Opción para buscar topics en los mensajes
    file_topics: "xxxxxx.csv"            # Nombre del fichero con los topics
---
...
```{r read_data, echo=FALSE, warning=TRUE}
...
if(ARS == TRUE) {
  ...
  msgs <- read_csv(
    ...
    cols_only( 
      ...
      msg_from_peer = col_character(),
      is_forward = col_integer(),
      is_reply = col_integer(),
      forward_msg_link = col_character(),
      forward_msg_from_peer_name = col_character(),
      url_description = col_character(),
      views = col_integer(),
      number_replies = col_integer(),
      number_forwards = col_integer(),
...
if(params$topics == TRUE){
  topics <- read_csv(paste0(data_path,params$file_topics), show_col_types = FALSE)
}
...
msgs_df <- msgs %>%
  ...
  mutate(relation = ifelse(is_forward == 1,"forward", "original")) %>%
  left_join(metadatos_df, by = c("channel_name" = "username")) %>%
  mutate(date = as.POSIXct(floor_date(date),"sec", tz = "")) %>%
...
Adicionalmente se etiquetan los canales que estén en el rango definido en el parámetro **min_suscribers**. Este parámetro se adecuará al alcance de la gráfica. Si no aparece ningún canal puede se debido a dos motivos:
...
if(params$zoom) {
  ...
  p <- msgs_vs_views(
        msgs_df,
        "zoom",
        params$date_ini_zoom,
        params$date_end_zoom,
        params$min_suscribers,
        params$max_overlaps )
  print(p)
  # Grabar la gráfica en un archivo
...
## msgs vs. forward send

### Periodo total

```{r msgs_vs_forward_out_total, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
# Llamamos a la función msgs_vs_forward_out
  p <- msgs_vs_forward_out(
    msgs_df,
    "total",
    min_date,
    max_date)
  print(p)
  # Grabar la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_forward_out_total.png"))
```

### Zoom

```{r msgs_vs_forward_out_zoom, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
if(params$zoom) {
  ...
  ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_forward_out_zoom.png"))
}
```

## msgs vs. forward received

### Periodo total

```{r msgs_vs_forward_in_total, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
# Llamamos a la función msgs_vs_forward
p <- msgs_vs_forward_in(
  msgs_df,
  "total",
  min_date,
  max_date
...
print(p)
# Grabar la gráfica en un archivo
ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_forward_in__total.png"))

```

### Zoom

```{r msgs_vs_forward_in_zoom, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
if(params$zoom) {
  # Llamamos a la función msgs_vs_forward
  p <- msgs_vs_forward_in(
    msgs_df,
  ...
  print(p)
  # Salvamos la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_forward_in_zoom.png"))
}
```
...
```{r msgs_vs_replies_total, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
# Llamamos a la función msgs_vs_forward
p <- msgs_vs_replies(
  msgs_df,
  "total",
...
print(p)
# Grabar la gráfica en un archivo
ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_replies_total.png"))

```

### Zoom

```{r msgs_vs_replies_zoom, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
if(params$zoom) {
  # Llamamos a la función msgs_vs_forward
  p <- msgs_vs_replies(
    msgs_df,
  ...
  print(p)
  # Salvamos la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_msgs_vs_replies_zoom.png"))
}
```
...
## Contenido de los msgs

### Periodo total

```{r contenido_msgs_total, echo=FALSE, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
p <-words_frequency(msgs_df, "Total", min_date, max_date)
print(p)
# Salvamos la gráfica en un archivo
ggsave(paste0(img_path,base_name_file_img,"_words_frequenc_total.png"))
```

### Zoom

```{r contenido_msgs_zoom, echo=FALSE, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
if(params$zoom) {
  p <-words_frequency(msgs_df, "zoom",  params$date_ini_zoom, params$date_end_zoom)
  print(p)
  # Salvamos la gráfica en un archivo
...
```{r topics_total, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE}
if(params$topics == TRUE){
  p <- spread_topics_msgs(msgs_df, "total",  min_date, max_date, topics$topics) 
  print(p)
  # Salvamos la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_topics_total.png"))
}
...
```{r topics_zoom, echo=FALSE, fig.height=5, fig.width=9, message=FALSE, warning=FALSE}
if(params$topics == TRUE & params$zoom){
  p <- spread_topics_msgs(msgs_df, "zoom", params$date_ini_zoom, params$date_end_zoom, topics$topics) 
  print(p)
  # Salvamos la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_topics_zoom.png"))
}
...
if(ARS & params$zoom) {
  ...
  print(p)
  # Salvamos la gráfica en un archivo
  ggsave(paste0(img_path,base_name_file_img,"_words_frequency_by_group_zoom.png"))
}
```
